...
![codestore demo](http://i.imgur.com/penUFkr.gif)

> [Documentation also available on codestorejs.com](https://codestorejs.com/documentation/)

# Getting Started
[![npm](https://img.shields.io/npm/dm/codestore.svg)](https://www.npmjs.com/package/codestore)
&nbsp;
[![npm](https://img.shields.io/npm/v/codestore.svg)](https://www.npmjs.com/package/codestore)
&nbsp;
[![codestore on slack](https://codestorejs-slack-vbeawnrrtk.now.sh/badge.svg)](https://codestorejs-slack-vbeawnrrtk.now.sh/)

## What is codestore?
codestore is what I like to call a "micro-generator framework." Now, I call it that because it is a small tool that gives you a simple way to generate code or any other type of flat text files in a consistent way. You see, we all create structures and patterns in our code (routes, controllers, components, helpers, etc). These patterns change and improve over time so when you need to create a NEW *insert-name-of-pattern-here*, it's not always easy to locate the files in your codebase that represent the current "best practice." That's where codestore saves you. With codestore, you have your "best practice" method of creating any given pattern in CODE. Code that can easily be run from the terminal by typing `codestore`. Not only does this save you from hunting around in your codebase for the right files to copy, but it also turns "the right way" into "the easiest way" to make new files.

If you boil codestore down to its core, it is basically glue code between  [inquirer](https://github.com/SBoudrias/Inquirer.js/) prompts and [handlebar](https://github.com/wycats/handlebars.js/) templates.

> This documentation is a work in progress. If you have great ideas, I'd love to hear them.

## Installation
### 1. Add codestore to your project
```
$ npm install --save-dev codestore
```
### 2. Install codestore globally (optional, but recommended for easy access)
```
$ npm install -g codestore
```
### 3. Create a codestorefile.js at the root of your project
``` javascript
module.exports = function (codestore) {
...
## Your First codestorefile
A codestorefile starts its life as a lowly node module that exports a function which accepts the `codestore` object as its first parameter.

``` javascript
module.exports = function (codestore) {};
```

The `codestore` object exposes the codestore api object which contains the `setGenerator(name, config)` function. This is the function that you use to (wait for it) create a generator for this codestorefile. When `codestore` is run from the terminal in this directory (or any sub-directory), a list of these generators will be displayed.
...
module.exports = function (codestore) {
	...
	codestore.setGenerator('controller', {
		...
		actions: [{
			type: 'add',
			path: 'src/{{name}}.js',
			templateFile: 'codestore-templates/controller.hbs'
		}]
	});
};
```
...
## Using Prompts
codestore uses the [inquirer.js](https://github.com/SBoudrias/Inquirer.js) library to gather user data. A list of [prompt types](https://github.com/SBoudrias/Inquirer.js/#prompt-types) can be found on the inquirer official website.

## CLI Usage
Once codestore is installed, and you have created a generator, you are ready to run codestore from the terminal. Running `codestore` with no parameters will present you with a list of generators to pick from. You can also run `codestore [generatorName]` to trigger a generator directly. If you did not install codestore globally, you will need to setup an npm script to run codestore for you.

### Bypassing Prompts
Once you get to know a project (and its generators) well, you may want to provide answers to the prompts when you run the generator. If I have (for instance) a `component` generator that has one prompt (name), I can run that generator using `codestore component "some component name"` and it will immediately execute as though I had typed "some component name" into the prompt. If that same generator had a second prompt, the same input would have resulted in the user being prompted for the second value.
...
![codestore bypass demo](https://media.giphy.com/media/3ov9jQ38ypmX4SuT60/giphy.gif)

> If you want to provide bypass input for the second prompt but not the first, you can use an underscore "\_" to skip the bypass (ie `codestore component _ "input for second prompt"`).

codestore comes with bypass logic built-in for standard inquirer prompts, but there are also ways to provide custom logic for how to handle user input for a specific prompt.

If you have published a 3rd party inquirer prompt plugin and would like to support bypass functionality for codestore users out of the box, that is covered in [another section of this documentation](#3rd-party-prompt-bypass).
...
$ codestore component "my component" react
$ codestore component -- --name "my component" --type react

## Bypassing only prompt 2 (will be prompted for name)
$ codestore component _ react
$ codestore component -- --type react
```

### Running a Generator Forcefully
By default codestore actions keep your files safe by failing when things look fishy. The most obvious example of this is not allowing an [`add`](#add) action to overwrite a file that already exists. codestore actions individually support the `force` property but you can also use the `--force` flag when running codestore from the terminal. Using the `--force` flag will tell every action to run forcefully. With great power...🕷
...
# codestorefile Api
The codestorefile api is the collection of methods that are exposed by the `codestore` object. Most of the work is done by [`setGenerator`](#setgenerator) but this section documents the other methods that you may also find useful in your codestorefile.

## Main Methods
These are the methods you will commonly use when creating a codestorefile. Other methods that are mostly for internal use are list in the [other methods](#other-methods) section.
...
[**load**](https://github.com/amwmedia/codestore/blob/master/codestore-load.md) | *Array[String], Object, Object* | | loads generators, helpers and/or partials from another codestorefile or npm module
...
``` javascript
module.exports = function (codestore) {
	codestore.setHelper('upperCase', function (text) {
		return text.toUpperCase();
	});

	// or in es6/es2015
	codestore.setHelper('upperCase', (txt) => txt.toUpperCase());
};
```
...
``` javascript
module.exports = function (codestore) {
	codestore.setPartial('myTitlePartial', '<h1>{{titleCase name}}</h1>');
	// used in template as {{> myTitlePartial }}
};
```

## setActionType
`setActionType` allows you to create your own actions (similar to `add` or `modify`) that can be used in your codestorefiles. These are basically highly reusable [custom action function](#custom-action-function-)s.
...
3) **codestore** | *[codestorefileApi](#codestorefile-api)* | The codestore api for the codestorefile where this action is being run

``` javascript
module.exports = function (codestore) {

	codestore.setActionType('doTheThing', function (answers, config, codestore) {
		// do something
	...
	codestore.setActionType('doTheAsyncThing', function (answers, config, codestore) {
		// do something
...
## setPrompt
[Inquirer](https://github.com/SBoudrias/Inquirer.js) provides many types of prompts out of the box, but it also allows developers to build prompt plugins. If you'd like to use a prompt plugin, you can register it with `setPrompt`. For more details see the [Inquirer documentation for registering prompts](https://github.com/SBoudrias/Inquirer.js#inquirerregisterpromptname-prompt). Also check out the [codestore community driven list of custom prompts](https://github.com/amwmedia/codestore/blob/master/inquirer-prompts.md).

``` javascript
const promptDirectory = require('inquirer-directory');
module.exports = function (codestore) {
	codestore.setPrompt('directory', promptDirectory);
	codestore.setGenerator('test', {
		prompts: [{
			type: 'directory',
			...
...
The following properties are the standard properties that codestore handles internally. Other properties will be required depending on the *type* of action. Also take a look at the [built-in actions](#built-in-actions).
...
**setWelcomeMessage** | *String* | | Customizes the displayed message that asks you to choose a generator when you run `codestore`.
...
**setcodestorefilePath** | *String* | | set the `codestorefilePath` value which is used internally to locate resources like template files
**getcodestorefilePath** | | *String* | returns the absolute path to the codestorefile in use
**getDestBasePath** | | *String* | returns the base path that is used when creating files
**setDefaultInclude** | *Object* | *Object* | sets the default config that will be used for this codestorefile if it is consumed by another codestorefile using `codestore.load()`
**getDefaultInclude** | *String* | *Object* | gets the default config that will be used for this codestorefile if it is consumed by another codestorefile using `codestore.load()`
...
# Built-In Actions
There are several types of built-in actions you can use in your [GeneratorConfig](#-interface-generatorconfig-). You specify which `type` of action  (all paths are based on the location of the codestorefile), and a template to use.
...
The `Add` and `Modify` actions will take care of almost every case that codestore is designed to handle. However, codestore does offer custom action functions for the node/js guru. A custom action function is a function that is provided in the actions array.
- Custom action functions are executed by codestore with the same [CustomAction](#-functionsignature-custom-action) function signature.
- codestore will wait for the custom action to complete before executing the next action.
- The function must let codestore known what’s happening through the return value. If you return a `Promise`, we won’t start other actions until the promise resolves. If you return a message (*String*), we know that the action is done and we’ll report the message in the status of the action.
- A custom action fails if the promise is rejected, or the function throws an `Exception`

_See the [example codestorefile](https://github.com/amwmedia/codestore/blob/master/example/codestorefile.js) for a sample synchronous custom action._

## Comments
Comment lines can be added to the actions array by adding a string in place of an action config object. Comments are printed to the screen when codestore comes to them and have no functionality of their own.

# Built-In Helpers
There are a few helpers that I have found useful enough to include with codestore. They are mostly case modifiers, but here is the complete list.
...
## Other Helpers
- **pkg**: look up a property from a package.json file in the same folder as the codestorefile.

# Taking it Further

There is not a lot needed to get up and running on some basic generators. However, if you want to take your codestore-fu further, read on young padawan.
...
If you have written an inquirer prompt plugin and want to support codestore's bypass functionality, the process is pretty simple. The plugin object that your prompt exports should have a `bypass` function. This `bypass` function will be run by codestore with the user's input as the first parameter and the prompt config object as the second parameter. The value that this function returns will be added to the answer data object for that prompt.
...
### Adding Bypass Support to Your codestorefile
If the 3rd party prompt plugin you are using does not support bypass by default, you can add the `bypass` function above to your prompt's config object and codestore will use it for handling bypass data for that prompt.
