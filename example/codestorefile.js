...
module.exports = function (codestore) {
	...
	// codestore.setWelcomeMessage('[CUSTOM]'.yellow + ' What can I do for you?');
	...
	codestore.addHelper('dashAround', (text) => '---- ' + text + ' ----');

	// formats an array of options like you would write
	// it, if you were speaking (one, two, and three)
	codestore.addHelper('wordJoin', function (words) {
		return words.join(', ').replace(/(:?.*),/, '$1, and');
	});

	codestore.addHelper('absPath', function (p) {
		return path.resolve(codestore.getPlopfilePath(), p);
	});

	// greet the user using a partial
	codestore.addPartial('salutation', '{{ greeting }}, my name is {{ properCase name }} and I am {{ age }}.');
	...
	// setGenerator creates a generator that can be run with "codestore generatorName"
	codestore.setGenerator('test', {
		...
		actions: [
			...
			function customAction(answers) {
				// move the current working directory to the codestore file path
				// this allows this action to work even when the generator is
				// executed from inside a subdirectory
				process.chdir(codestore.getPlopfilePath());
				...
				// you can use codestore.renderString to render templates
				existsMsg = codestore.renderString(existsMsg, answers);
				copiedMsg = codestore.renderString(copiedMsg, answers);
	...
	// codestore.addPrompt('directory', require('inquirer-directory'));

	codestore.setGenerator('custom-prompt', {
		...
		prompts: [
			{
				...
				basePath: codestore.getPlopfilePath()
			}
		],
		actions: [
			function(data) {
				...
				template: '{{absPath path}}/{{fileName}} codestoreped!'
			}
		]
	});
	...
	codestore.setGenerator('dynamic actions', {
